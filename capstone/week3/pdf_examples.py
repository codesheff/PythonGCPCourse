...
#There's a few tools in Python that let you generate PDFs with the content that you want. 
# Here, we'll learn about one of them: ReportLab. ReportLab has a lot of different features for creating PDF documents. 
# We'll cover just the basics here, and give you pointers for more information at the end.

# For our examples, we'll be mostly using the high-level classes and methods in the Page Layout and Typography Using Scripts (PLATYPUS) part of 
...
# The report object that we just created will end up generating a PDF using the filename /tmp/report.pdf. 
# Now, let's add some content to it! 
# We'll create a title, some text in paragraphs, and some charts and images. 
# For that, we're going to use what reportlab calls Flowables. 
# Flowables are sort of like chunks of a document that reportlab can arrange to make a complete report. 
# Let's import some Flowable classes.

from reportlab.platypus import Paragraph, Spacer, Table, Image

# Each of these items (Paragraph, Spacer, Table, and Image) are classes that build individual elements in the final document. 
...
# Let's spice this up by adding a Table. 
...
# Okay, it worked! It's not very easy to read, though. 
...
#Alright, and with that, you've seen a few examples of what we can do with the ReportLab library.  
# There's a ton more things that can be done that we won't cover here. 
# You'll want to refer to the ReportLab User Guide for more details on the features we've seen, and to see what else you can create with it.
