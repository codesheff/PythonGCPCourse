...
# The set_content() method also automatically added a couple of headers that the email infrastructure will use when sending this message to another machine. 
# Remember in an earlier course, when we talked about character encodings? 
...
# Now, what about this other header? What is MIME? We'll learn about that next!
...
#Remember, email messages are made up completely of strings. 
# When you add an attachment to an email, whatever type the attachment happens to be, it’s encoded as some form of text. 
...
#As we called out, to send emails, our computers use the Simple Mail Transfer Protocol (SMTP). 
# This protocol specifies how computers can deliver email to each other. 
# There are certain steps that need to be followed to do this correctly. 
# But, as usual, we won't do this manually; we’ll send the message using the built-in smtplib Python module. 
...
# If you’re using a Linux computer, you might already have a configured SMTP server like postfix or sendmail. 
# But maybe not. Let's create a smtplib.SMTP object and try to connect to the local machine.
...
# Now that we’ve made a connection to the SMTP server, the next thing we need to do is authenticate to the SMTP server. Typically, email providers wants us to provide a username and password to connect. 
